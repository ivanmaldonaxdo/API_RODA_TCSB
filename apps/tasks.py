            ...
            if datetime.datetime.now().date()==self.cronconfig.fecha:
                ...
                for cli in self.clientes:
                    for ser in self.servicios:
                        url4 =  'http://3.219.56.115/cron/verificar_status/'
                        cookie = settings.CRON_CREDENCIAL
                        estado = requests.get(url=url4, cookies={'jwt':cookie})
                        con = estado.json()
                        if con['status'] == True:
                            url =  'http://3.219.56.115/documentos/search_docs/'
                            cookie = settings.CRON_CREDENCIAL
                            body={
                                'folio': '', 
                                'tipo_servicio': ser,
                            ...
                            if search.status_code==200:
                                for bol in data:
                                    url2 =  'http://3.219.56.115/cron/verificar_status/'
                                    cookie = settings.CRON_CREDENCIAL
                                    ...
                                    if con['status'] == True:
                                        ...
                                        url3 =  'http://3.219.56.115/documentos/process_docs/'
